...
const (
	dexCertGeneratedName = "kubic-dex-cert"
)
...
func (cert *DexCertificate) GetFrom(instance *kubicv1beta1.DexConfiguration) error {
	...
	if len(instance.Spec.Certificate.Name) > 0 {
		...
		name, namespace = dexCertGeneratedName, metav1.NamespaceSystem
	}
...
func (cert *DexCertificate) CreateOrUpdate(deployment *DexDeployment) error {
	...
	defaultAddress, err := cert.reconciler.kubicCfg.GetBindIP()
	if err != nil {
		return err
	}
	...
	certNames := []string{
		dexServiceName,
		cert.reconciler.kubicCfg.Network.Dns.ExternalFqdn,
		cert.reconciler.kubicCfg.GetServiceDNSName(deployment),
		deployment.GetName(),
	}
