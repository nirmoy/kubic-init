...
func Add(mgr manager.Manager, kubicCfg *config.KubicInitConfiguration) error {
	return add(mgr, newReconciler(mgr, kubicCfg))
}

// newReconciler returns a new reconcile.Reconciler
func newReconciler(mgr manager.Manager, kubicCfg *config.KubicInitConfiguration) reconcile.Reconciler {
...
func (r *ReconcileDexConfiguration) Reconcile(request reconcile.Request) (reconcile.Result, error) {
	...
	sharedPasswords, err := crypto.NewSharedPasswordsSet(r.Clientset, dexSharedPasswordsSecrets, instance.GetNamespace())
	if err != nil {
		return reconcile.Result{}, err
	}
	...
	if finalizing {
		err = r.reconcileRemoval(instance, deployment, configMap, sharedPasswords)
		r.finalizerDone(instance)
	} else {
		rr, err = r.reconcileInstance(instance, deployment, configMap, sharedPasswords)
	}
...
func (r *ReconcileDexConfiguration) reconcileInstance(instance *kubicv1beta1.DexConfiguration, deployment *DexDeployment, configMap *DexConfigMap, sharedPasswords crypto.SharedPasswordsSet) (reconcile.Result, error) {
	var err error
	...
	if len(connectors) == 0 && deployment.IsRunning() {
		...
		if err = r.reconcileRemoval(instance, deployment, configMap, sharedPasswords); err != nil {
			return reconcile.Result{}, err
		}
		return reconcile.Result{}, nil
	}
	...
	if err = configMap.CreateLocal(connectors, sharedPasswords); err != nil {
	...
	if err := sharedPasswords.CreateOrUpdateToSecrets(r.Clientset); err != nil {
		return reconcile.Result{}, err
	}
	instance.Status.StaticPasswords = map[string]corev1.SecretReference{}
	for name, password := range sharedPasswords {
		instance.Status.StaticPasswords[name] = password.AsSecretReference()
	}
	r.EventRecorder.Event(instance, corev1.EventTypeNormal,
		"Deploying", fmt.Sprintf("Created %d Secrets for shared passwords for '%s'",
			len(sharedPasswords), instance.GetName()))
...
func (r *ReconcileDexConfiguration) reconcileRemoval(instance *kubicv1beta1.DexConfiguration, deployment *DexDeployment, configMap *DexConfigMap, sharedPasswords crypto.SharedPasswordsSet) error {
	var err error
	...
	// remove the sharedPasswords and the certificate
	for pn, password := range sharedPasswords {
		...
		delete(instance.Status.StaticPasswords, pn)
	}
