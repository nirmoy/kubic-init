...
func createOrUpdateDexService(cli clientset.Interface, nodeport int) error {
	...
	dexService.Spec.Ports[0].NodePort = int32(nodeport)

	glog.V(3).Infof("[kubic] creating Service '%s'", dexService.GetName())
	if _, err := kubicclient.CreateOrUpdateService(cli, &dexService); err != nil && !apierrors.IsAlreadyExists(err) {
		return err
	} else if err := kubicclient.WaitForObject(cliREST, &dexService); err != nil {
		return err
	}
	return nil
}
...
func createOrUpdateDexNetworkPolicy(cli clientset.Interface) error {
	// try to replicate the old behaviour in
	// https://github.com/kubic-project/salt/blob/master/salt/addons/dex/manifests/30-network-policy.yaml
