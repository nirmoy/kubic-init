...
import (
	...
	caaspboot "github.com/inercia/caasp-init/pkg/bootstrap"
)
...
// Version provides the version information of caasp-init
type Version struct {
	ClientVersion *apimachineryversion.Info `json:"clientVersion"`
}

// The caasp-init configuration
type CaasInitConfiguration struct {
	Seeder string
}

// Load a CaaSP configuration file, setting some default values
...
// newBootstrapCmd returns a "caasp-init bootstrap" command.
func newBootstrapCmd(out io.Writer) *cobra.Command {
	caaspCfg := &CaasInitConfiguration{}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			caaspCfg, err = ConfigFileAndDefaultsToCaasConfig(caaspCfgFile, caaspCfg)
			...
			if len(caaspCfg.Seeder) > 0 {

				glog.V(1).Infoln("[caas] joining the seeder at %s", caaspCfg.Seeder)
				...
				err = caaspboot.Join(nodeCfg, ignorePreflightErrorsSet)
				kubeadmutil.CheckErr(err)
				...
				err = caaspboot.Init(externalMasterCfg, cfg, bto, ignorePreflightErrorsSet)
				kubeadmutil.CheckErr(err)
	...
	flagSet.StringVar(&caaspCfgFile, "config", "",
		"Path to caasp config file.")
	flagSet.StringVar(&kubeadmCfgFile, "kubeadm-config", "",
		"Path to kubeadm config file. WARNING: Usage of a configuration file is experimental.")
	flagSet.StringVar(&caaspCfg.Seeder, "seeder", "",
		"Cluster seeder.")
...
func newCmdVersion(out io.Writer) *cobra.Command {
	cmd := &cobra.Command{
		Use:   "version",
		Short: "Print the version of caasp-init",
		Run: func(cmd *cobra.Command, args []string) {
			...
			fmt.Fprintf(out, "caasp-init version: %#v\n", v.ClientVersion)
		},
	}
...
func main() {
	...
	cmds := &cobra.Command{
		Use:   "caasp-init",
		Short: "caasp-init: easily bootstrap a secure Kubernetes cluster",
		Long: dedent.Dedent(`
			caasp-init: easily bootstrap a secure Kubernetes cluster.
		`),
	}
